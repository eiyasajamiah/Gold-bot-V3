...
import os, time, json, logging, hashlib, threading
...
# ── قراءة Environment Variables (Railway / Render / أي سيرفر) ──
_ENV_KEYS = [
    "TELEGRAM_TOKEN", "TELEGRAM_CHAT_ID", "CLAUDE_API_KEY",
    "CLAUDE_PROVIDER", "CLAUDE_MODEL", "SYMBOL",
]
for _k in _ENV_KEYS:
...
MAX_TP    = float(cfg.get("MAX_TP_POINTS", 20))
...
def _handle_cmd(cid: str, nm: str, txt: str):
    cmds = {
        "/start":       (S_ALL,
            ...
            "يجمع 8 استراتيجيات + ذكاء اصطناعي في منظومة واحدة\n\n"
            "📡 <b>الاستراتيجيات:</b>\n"
            "  ① V7  — 15 مؤشر × 4 إطارات زمنية\n"
            "  ② SMC — Smart Money Concepts\n"
            "  ③ OBinOB — كتل الأوامر المتداخلة\n"
            "  ④ جلسة آسيا — انعكاس 02:00 UTC\n"
            "  ⑤ Range — تداول النطاق\n"
            "  ⑥ Fibonacci — تحليل فيبوناتشي الشامل\n"
            "  ⑦ Reversal — أنماط الانعكاس\n"
            "  ⑧ Single Candle — تحليل الشمعة الواحدة\n"
            "  🤖 Claude AI — محلّل ذكي اختياري\n\n"
            "🆕 <b>جديد في v4.0:</b>\n"
            "  📊 VWAP + Squeeze + Supply/Demand\n"
            "  🎯 كاش ذكي لسرعة أعلى\n"
            "  🔍 Divergence محسّن بـ Swing Points\n\n"
...
STRAT_NAMES = [
    ...
    "V7_VWAP","V7_Squeeze","V7_SupplyDemand",
    ...
    "VWAP_Signal","Squeeze_Signal","SD_Zone_Signal",
]
...
_CACHE_TTL = 55  # seconds — cache for 55s to avoid duplicate calls within same cycle
...
def fetch(sym: str, period: str, interval: str) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if df is None or df.empty:
            _data_cache[cache_key] = (now, None)
            return None
        ...
        result = df.reset_index(drop=True) if len(df) >= 50 else None
...
def clear_cache():
    """Clears the data cache — call at the start of each analysis cycle"""
    _data_cache.clear()
...
def detect_divergence(price: pd.Series, indicator: pd.Series,
    ...
    """
    Bullish Divergence: سعر يصنع قاعاً أدنى لكن المؤشر يصنع قاعاً أعلى
    Bearish Divergence: سعر يصنع قمة أعلى لكن المؤشر يصنع قمة أدنى
    Returns: (bull_div, bear_div)
    — محسّنة: تستخدم Swing Points بدل مقارنة بسيطة
    """
    p = price.tail(lookback).values
    ind = indicator.tail(lookback).values
    n = len(p)
    if n < 15: return False, False
    ...
    # Bullish Divergence: قاعان متتاليان — السعر أدنى لكن المؤشر أعلى
    if len(swing_lows) >= 2:
        sl1, sl2 = swing_lows[-2], swing_lows[-1]
        if p[sl2] < p[sl1] and ind[sl2] > ind[sl1]:
            bull_div = True

    # Bearish Divergence: قمتان متتاليتان — السعر أعلى لكن المؤشر أدنى
    if len(swing_highs) >= 2:
        sh1, sh2 = swing_highs[-2], swing_highs[-1]
        if p[sh2] > p[sh1] and ind[sh2] < ind[sh1]:
            bear_div = True
...
# ══════════════════════════════════════════════════════════════════
def vwap_fn(df: pd.DataFrame) -> pd.Series:
    """يحسب VWAP — السعر المرجح بالحجم"""
...
def vwap_bands(df: pd.DataFrame, std_mult: float = 2.0) -> Tuple[pd.Series, pd.Series, pd.Series]:
    """VWAP مع نطاقات الانحراف المعياري"""
    vw = vwap_fn(df)
    tp = (df["High"] + df["Low"] + df["Close"]) / 3
    deviation = ((tp - vw) ** 2 * df["Volume"]).cumsum() / (df["Volume"].cumsum() + 1e-9)
    std = np.sqrt(deviation)
    return vw, vw + std_mult * std, vw - std_mult * std


# ══════════════════════════════════════════════════════════════════
# Keltner Channel — لاستراتيجية Squeeze
# ══════════════════════════════════════════════════════════════════
def keltner_fn(df: pd.DataFrame, n: int = 20, mult: float = 1.5) -> Tuple[pd.Series, pd.Series, pd.Series]:
...
def momentum_squeeze(df: pd.DataFrame) -> Tuple[bool, float, str]:
    """
    Momentum Squeeze: عندما Bollinger داخل Keltner → ضغط → انفجار قادم
    يُرجع: (squeeze_on, momentum_value, description)
    """
    bb_upper, bb_mid, bb_lower = bb_fn(df["Close"], 20, 2)
    kc_upper, kc_mid, kc_lower = keltner_fn(df, 20, 1.5)

    # هل BB داخل KC؟
    squeeze_on = bool(bb_upper.iloc[-1] < kc_upper.iloc[-1] and bb_lower.iloc[-1] > kc_lower.iloc[-1])

    # قيمة الزخم — الفرق بين السعر والمتوسط
    momentum = float(df["Close"].iloc[-1] - kc_mid.iloc[-1])

    # هل كان هناك squeeze في الشمعتين السابقتين؟
    prev_sq = (bb_upper.iloc[-2] < kc_upper.iloc[-2] and bb_lower.iloc[-2] > kc_lower.iloc[-2])
    squeeze_release = prev_sq and not squeeze_on

    if squeeze_release:
        desc = f"💥 Squeeze Release! زخم={'صاعد ↑' if momentum > 0 else 'هابط ↓'} ({momentum:+.2f})"
    elif squeeze_on:
        desc = f"🔴 Squeeze مستمر — انتظر الانفجار (زخم={momentum:+.2f})"
    else:
        desc = f"⬜ لا ضغط — زخم={'إيجابي' if momentum > 0 else 'سلبي'} ({momentum:+.2f})"

    return squeeze_on, momentum, desc


# ══════════════════════════════════════════════════════════════════
# Supply/Demand Zones — مناطق العرض والطلب
# ══════════════════════════════════════════════════════════════════
def detect_supply_demand(df: pd.DataFrame, lookback: int = 60) -> dict:
    """
    يكشف مناطق العرض (بيع) والطلب (شراء) بناءً على:
    - شموع قوية تليها حركة عنيفة
    - مناطق لم يعد إليها السعر بعد
    """
    sub = df.tail(lookback)
    at = atr_fn(sub).iloc[-1]
    price = float(sub["Close"].iloc[-1])
    n = len(sub)

    demand_zones = []  # مناطق شراء
    supply_zones = []  # مناطق بيع
    ...
    for i in range(2, n - 3):
        ...
        # شمعة هبوط قوي تليها صعود قوي → منطقة طلب
        next_move = sub["Close"].iloc[i+1:i+4].max() - c["Low"]
        if c["Close"] < c["Open"] and next_move > at * 1.5:
            zone_hi = max(c["Open"], c["Close"])
            zone_lo = c["Low"]
            # هل لم يعد السعر لهذه المنطقة؟
            revisited = any(sub["Low"].iloc[j] <= zone_hi for j in range(i+4, min(i+20, n)))
            if not revisited or abs(price - zone_lo) / price < 0.01:
                demand_zones.append({
                    "hi": round(zone_hi, 2), "lo": round(zone_lo, 2),
                    "mid": round((zone_hi + zone_lo) / 2, 2),
                    "strength": round(next_move / at, 1),
                    "fresh": not revisited, "age": n - i
                })

        # شمعة صعود قوي تليها هبوط قوي → منطقة عرض
        next_drop = c["High"] - sub["Close"].iloc[i+1:i+4].min()
        if c["Close"] > c["Open"] and next_drop > at * 1.5:
            zone_hi = c["High"]
            zone_lo = min(c["Open"], c["Close"])
            revisited = any(sub["High"].iloc[j] >= zone_lo for j in range(i+4, min(i+20, n)))
            if not revisited or abs(price - zone_hi) / price < 0.01:
                supply_zones.append({
                    "hi": round(zone_hi, 2), "lo": round(zone_lo, 2),
                    "mid": round((zone_hi + zone_lo) / 2, 2),
                    "strength": round(next_drop / at, 1),
                    "fresh": not revisited, "age": n - i
                })
...
def _s_vwap(df: pd.DataFrame) -> Tuple[int,str]:
    """VWAP — السعر المرجح بالحجم"""
    if len(df) < 20: return 0, "VWAP: بيانات غير كافية"
    vw, vw_upper, vw_lower = vwap_bands(df)
    c = df["Close"].iloc[-1]
    vw_val = vw.iloc[-1]
    dist_pct = (c - vw_val) / vw_val * 100

    if c < vw_lower.iloc[-1]:
        return 1, f"VWAP تحت النطاق السفلي — ذروة بيع ({dist_pct:+.2f}%)"
    if c > vw_upper.iloc[-1]:
        return -1, f"VWAP فوق النطاق العلوي — ذروة شراء ({dist_pct:+.2f}%)"
    if c > vw_val and c > df["Close"].iloc[-2]:
        return 1, f"VWAP فوق المتوسط وصاعد ({dist_pct:+.2f}%)"
    if c < vw_val and c < df["Close"].iloc[-2]:
        return -1, f"VWAP تحت المتوسط وهابط ({dist_pct:+.2f}%)"
    return 0, f"VWAP محايد ({dist_pct:+.2f}%)"

def _s_squeeze(df: pd.DataFrame) -> Tuple[int,str]:
    """Momentum Squeeze — Bollinger داخل Keltner"""
    if len(df) < 25: return 0, "Squeeze: بيانات غير كافية"
    sq_on, momentum, desc = momentum_squeeze(df)
    if not sq_on and momentum > 0 and abs(momentum) > atr_fn(df).iloc[-1] * 0.3:
        return 1, f"Squeeze Release صاعد! زخم={momentum:+.2f}"
    if not sq_on and momentum < 0 and abs(momentum) > atr_fn(df).iloc[-1] * 0.3:
        return -1, f"Squeeze Release هابط! زخم={momentum:+.2f}"
    if sq_on:
        return 0, f"Squeeze مستمر — انتظر الانفجار (زخم={momentum:+.2f})"
    return 0, desc

def _s_supply_demand(df: pd.DataFrame) -> Tuple[int,str]:
    """Supply/Demand Zones — مناطق العرض والطلب"""
    if len(df) < 30: return 0, "S/D: بيانات غير كافية"
    sd = detect_supply_demand(df, 60)
    price = df["Close"].iloc[-1]
    at = atr_fn(df).iloc[-1]

    nd = sd["nearest_demand"]
    ns = sd["nearest_supply"]

    # السعر عند منطقة طلب
    if nd and abs(price - nd["mid"]) < at * 0.8:
        fresh = "طازجة ✅" if nd["fresh"] else "مُعاد اختبارها"
        return 1, f"عند منطقة طلب {nd['lo']:.2f}–{nd['hi']:.2f} (قوة={nd['strength']}× {fresh})"

    # السعر عند منطقة عرض
    if ns and abs(price - ns["mid"]) < at * 0.8:
        fresh = "طازجة ✅" if ns["fresh"] else "مُعاد اختبارها"
        return -1, f"عند منطقة عرض {ns['lo']:.2f}–{ns['hi']:.2f} (قوة={ns['strength']}× {fresh})"

    # السعر بين مناطق
    demand_near = f"أقرب طلب={nd['mid']:.2f}" if nd else "لا طلب"
    supply_near = f"أقرب عرض={ns['mid']:.2f}" if ns else "لا عرض"
    return 0, f"S/D: {demand_near} | {supply_near}"
...
V7_STRATS = [
    ...
    ("V7_VWAP",           _s_vwap,     2),
    ("V7_Squeeze",        _s_squeeze,  3),
    ("V7_SupplyDemand",   _s_supply_demand, 2),
]
...
def analyze_v7() -> dict:
    ...
    if vote and df1h is not None:
        ...
        if vote==1:
            ...
            tp1=min(pr+at*1.5,  pr+MAX_TP*0.35)
            tp2=min(pr+at*ATR_TP, pr+MAX_TP*0.65)
            tp3=min(pr+at*4.5,  pr+MAX_TP)
            tp3=min(tp3, fb["0"])
            tp=tp2
        else:
            sl=min(pr+at*ATR_SL, fb["382"])
            tp1=max(pr-at*1.5,  pr-MAX_TP*0.35)
            tp2=max(pr-at*ATR_TP, pr-MAX_TP*0.65)
            tp3=max(pr-at*4.5,  pr-MAX_TP)
            tp3=max(tp3, fb["1000"])
            tp=tp2
        entry=pr
        rr=round(abs(tp-entry)/(abs(sl-entry)+1e-9),2)
        vol_ok=df1h["Volume"].iloc[-1]>df1h["Volume"].rolling(20).mean().iloc[-1]*1.05
        # rr يُعاد حسابه بعد تطبيق MAX_TP في aggregate() - لا نرفض هنا
        # if rr<MIN_RR: vote=0
...
def analyze_smc(candles: list) -> dict:
    ...
    if score>=SMC_SC:
        ...
        risk=entry-sl
        tp1=min(entry+risk*1.5, entry+MAX_TP*0.35)
        tp2=min(entry+risk*2.5, entry+MAX_TP*0.65)
        tp3=min(entry+risk*4.0, entry+MAX_TP)
        tp=tp2
        ...
        risk=sl-entry
        tp1=max(entry-risk*1.5, entry-MAX_TP*0.35)
        tp2=max(entry-risk*2.5, entry-MAX_TP*0.65)
        tp3=max(entry-risk*4.0, entry-MAX_TP)
        tp=tp2
...
ASIA_MIN_IMPULSE_PIPS = 3.0  # أدنى حركة impulse بالدولار  # أدنى حركة impulse بالدولار
...
RANGE_BUFFER_PCT    = 0.002  # 0.2% هامش لتحديد الموقع
RANGE_BREAKOUT_CONFIRM = 2   # عدد الشموع لتأكيد الكسر
RANGE_MIN_WIDTH     = 0.005  # 0.5% أدنى عرض للنطاق
...
def analyze_reversal() -> dict:
    ...
    # if rr < 1.3: vote = 0  # RR يُعاد حسابه بعد MAX_TP في aggregate()
...
SQ_MIN_QUALITY    = 40   # أدنى جودة لإرسال الإشارة (خُفِّض من 55 لزيادة الإشارات)
...
def calc_signal_quality(agg: dict, v7: dict, smc: dict, ob,
    ...
    # ── 7.5 VWAP / Squeeze / Supply-Demand ──
    vwap_s = strats.get("V7_VWAP", {})
    if vwap_s.get("signal") == d:
        score += 5; bonuses.append(f"✅ VWAP يدعم الاتجاه (+5)")

    squeeze_s = strats.get("V7_Squeeze", {})
    if squeeze_s.get("signal") == d:
        score += 7; bonuses.append(f"✅ Squeeze Release في الاتجاه (+7)")
    elif squeeze_s.get("signal") == 0 and "مستمر" in squeeze_s.get("reason", ""):
        score += 3; bonuses.append(f"⏳ Squeeze مستمر — انفجار قريب (+3)")

    sd_s = strats.get("V7_SupplyDemand", {})
    if sd_s.get("signal") == d:
        score += 6; bonuses.append(f"✅ منطقة عرض/طلب تدعم (+6)")
...
def analyze_single_candle() -> dict:
    ...
    if final_vote and df1h_ref is not None:
        ...
        if final_vote == 1:
            ...
            tp2 = round(price - at_1h * 3.5, 2)
        rr = round(abs(tp2 - entry) / (abs(sl - entry) + 1e-9), 2)
        # if rr < 1.3: final_vote = 0  # RR يُعاد حسابه بعد MAX_TP
...
# ── إعدادات ──
COOLDOWN_MIN      = 15    # دقائق هدوء بين الإشارات (خُفِّض من 30)
...
def is_sideways_market(v7: dict) -> bool:
    ...
    if max(buy_w, sell_w) < max_w * 0.15:  # خُفِّض من 0.25 لتقليل الرفض
        return True
    return False
...
_AI_PROVIDERS = {
    # ── مجانية تماماً بدون مفتاح ────────────────────────
    "pollinations": {
        # مجاني 100% — بدون مفتاح — بدون تسجيل
        "url":   "https://text.pollinations.ai/openai",
        "model": cfg.get("CLAUDE_MODEL","") or "openai-large",
        "fmt":   "openai",
        "headers": lambda key: {
            "Content-Type": "application/json",
        },
        "no_key": True,  # لا يحتاج مفتاحاً
    },
    # ── مجانية ──────────────────────────────────────────
    "groq": {
...
_AUTO_ORDER = ["pollinations", "groq", "openrouter", "gemini", "ppq", "anthropic"]
...
def ai_analyze(v7: dict, smc: dict, ob: dict, agg: dict, price: float) -> dict:
    ...
    # السماح بالعمل بدون مفتاح إذا كان المزوّد لا يحتاجه (مثل pollinations)
    if not api_key:
        no_key_providers = [n for n,p in _AI_PROVIDERS.items() if p.get("no_key")]
        if provider == "auto":
            provider = no_key_providers[0] if no_key_providers else "pollinations"
            log.info(f"[AI] لا مفتاح — استخدام {provider} (مجاني بدون مفتاح)")
...
def merge_with_ai(agg: dict, ai: dict) -> dict:
    ...
    # ══ تطبيق حد الأهداف MAX_TP بعد دمج AI ══
    d_fin = agg.get("d", 0)
    if d_fin and entry is not None:
        _sl_max = (MAX_TP * 0.65) / max(MIN_RR, 1.2)
        if d_fin == 1:
            tp1 = min(tp1 if tp1 is not None else entry + MAX_TP * 0.35, entry + MAX_TP * 0.35)
            tp2 = min(tp2 if tp2 is not None else entry + MAX_TP * 0.65, entry + MAX_TP * 0.65)
            tp3 = min(tp3 if tp3 is not None else entry + MAX_TP,        entry + MAX_TP)
            if sl is not None and (entry - sl) > _sl_max:
                sl = round(entry - _sl_max, 2)
        else:
            tp1 = max(tp1 if tp1 is not None else entry - MAX_TP * 0.35, entry - MAX_TP * 0.35)
            tp2 = max(tp2 if tp2 is not None else entry - MAX_TP * 0.65, entry - MAX_TP * 0.65)
            tp3 = max(tp3 if tp3 is not None else entry - MAX_TP,        entry - MAX_TP)
            if sl is not None and (sl - entry) > _sl_max:
                sl = round(entry + _sl_max, 2)
        if sl is not None and tp2 is not None:
            rr = round(abs(tp2 - entry) / (abs(sl - entry) + 1e-9), 2)
...
def aggregate(v7: dict, smc: dict, ob: dict, asia: dict = None, rng: dict = None, fib: dict = None, rev: dict = None, sc: dict = None) -> dict:
    ...
    sc_cnt=sum(1 for v in votes.values() if v==-1)
    d=0
    if bc>=UNI_AGR: d=1
    elif sc_cnt>=UNI_AGR: d=-1
    conf=0
    if d:
        ag=max(bc,sc_cnt); conf=ag*25
    ...
    ag_cnt=max(bc,sc_cnt)
    lbl="شراء 🟢 LONG" if d==1 else ("بيع 🔴 SHORT" if d==-1 else "⏳ لا إشارة")
    log.info(f"[AGG] votes={votes} d={d} agree={ag_cnt}/3 conf={conf}%")

    # ══ تطبيق حد الأهداف MAX_TP على جميع الإشارات ══
    # مع ضبط SL تلقائياً لضمان R:R مقبول (المشكلة الجذرية: MAX_TP يُقيّد TP لكن SL يبقى ضخماً)
    if d and entry is not None:
        _sl_max = (MAX_TP * 0.65) / max(MIN_RR, 1.2)   # أقصى مسافة SL تُبقي R:R مقبولاً
        if d == 1:  # شراء
            tp1 = min(tp1 if tp1 is not None else entry + MAX_TP * 0.35, entry + MAX_TP * 0.35)
            tp2 = min(tp2 if tp2 is not None else entry + MAX_TP * 0.65, entry + MAX_TP * 0.65)
            tp3 = min(tp3 if tp3 is not None else entry + MAX_TP,        entry + MAX_TP)
            tp  = min(tp  if tp  is not None else entry + MAX_TP * 0.65, entry + MAX_TP * 0.65)
            # ضبط SL: لا يبعد أكثر من _sl_max لضمان R:R مقبول
            if sl is not None and (entry - sl) > _sl_max:
                sl = round(entry - _sl_max, 2)
        else:        # بيع
            tp1 = max(tp1 if tp1 is not None else entry - MAX_TP * 0.35, entry - MAX_TP * 0.35)
            tp2 = max(tp2 if tp2 is not None else entry - MAX_TP * 0.65, entry - MAX_TP * 0.65)
            tp3 = max(tp3 if tp3 is not None else entry - MAX_TP,        entry - MAX_TP)
            tp  = max(tp  if tp  is not None else entry - MAX_TP * 0.65, entry - MAX_TP * 0.65)
            # ضبط SL: لا يبعد أكثر من _sl_max لضمان R:R مقبول
            if sl is not None and (sl - entry) > _sl_max:
                sl = round(entry + _sl_max, 2)
        # إعادة حساب R:R بعد تعديل الأهداف والـ SL
        if sl is not None and tp2 is not None:
            rr = round(abs(tp2 - entry) / (abs(sl - entry) + 1e-9), 2)
...
def msg_signal(v7: dict, smc: dict, ob: dict, agg: dict, trigger_reasons: list, sq: dict = None, cm: dict = None) -> str:
    ...
    if sq and cm:
        ...
        # قسم مخاطرة افتراضي يظهر دائماً عند غياب sq/cm
        _risk_pts = abs((agg.get("entry") or 0) - (agg.get("sl") or 0))
        lines.append(
            "\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "🛡️ <b>إدارة المخاطر — مهم جداً:</b>\n"
            "   ⚠️ خاطر بـ <b>1-2% فقط</b> من رأس المال في هذه الصفقة\n"
            f"   📏 المسافة للـ Stop Loss: <b>{_risk_pts:.2f}$</b>\n"
            "   🔒 ضع الـ SL إلزامياً قبل الدخول\n"
            "   💡 TP1 أولاً — ثم حرّك SL للدخول (Break Even)\n"
            "   🚫 لا تضاعف الصفقة عند الخسارة"
        )
...
def msg_analysis(v7: dict, smc: dict, ob: dict, price: float, asia: dict = None, rng: dict = None, fib: dict = None, sc: dict = None) -> str:
    ...
    lines+=[
        "",f"⚙️ الإشارة عند توافق {UNI_AGR}+ من 3  |  #Gold #XAU_USD #إشارات_ذهب"
    ]
...
def main():
    ...
    while True:
        ...
        clear_cache()  # تنظيف كاش البيانات في بداية كل دورة
        ...
        try:
            ...
            log.exception("SQ error")
            sq_res = {"score":60,"grade":"متوسطة","size_lbl":"1% من الرأسمال","bonuses":[],"maluses":[],"tradeable":True}
        try:
            cm_res = calc_counter_move_plan(agg,smc.get("price",cur_price),_at_v)
        except Exception:
            log.exception("CM error")
            _e = agg.get("entry") or cur_price
            _s = agg.get("sl") or (_e - 10 if agg.get("d")==1 else _e + 10)
            _t1= agg.get("tp1") or (_e + 7 if agg.get("d")==1 else _e - 7)
            cm_res = {
                "defend_plan":[
                    f"① راقب الزخم عند الارتداد",
                    f"② عند تحقيق TP1 ({_t1:.2f}): أغلق 50% من المركز",
                    f"③ انقل SL للتعادل بعد 40% من المسافة للـ TP1",
                    f"④ Trailing Stop يتبع السعر بعد TP1",
                ],
                "scenarios":[
                    {"type":"ارتداد خفيف","level":round((_e+_s)/2,2),"color":"🟡",
                     "action":"احتفظ بالمركز — الاتجاه سليم"},
                    {"type":"كسر Stop Loss","level":_s,"color":"🔴",
                     "action":"أغلق فوراً — لا تتمسك بالأمل"},
                ],
                "reentry_zone":(_e-5, _e+5),
                "invalidation": round(_s * (0.998 if agg.get("d")==1 else 1.002), 2),
            }
        ...
        if agg["d"]!=0 and agg["entry"] is not None:
            ...
            if sid==last_sig:
                ...
                if not send_ok:
                    ...
                    # أرسل التحليل للمشتركين بدلاً من تجاهل الدورة بالكامل
...
# ══════════════════════════════════════════════════════════════════
# HEALTH CHECK SERVER — لإبقاء Render.com نشطاً
# ══════════════════════════════════════════════════════════════════
class _HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Gold Bot OK")
    def log_message(self, *a): pass  # أخفِ logs الـ HTTP
