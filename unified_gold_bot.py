...
def _handle_cmd(cid: str, nm: str, txt: str):
    cmds = {
        "/start":       (S_ALL, "✅ <b>اشتركت في بوت الذهب الموحّد v3.0!</b>\n\n"
                         "📡 3 استراتيجيات + مشغّل ذكي:\n"
                         "  ① V7  — 10 مؤشرات × 4 إطارات + Divergence\n"
                         "  ② SMC — BOS/CHoCH/OB/FVG/Liquidity/MSS\n"
                         "  ③ OBinOB — كتل الأوامر المتداخلة\n\n"
                         "⚙️ <b>الأوامر:</b>\n"
                         "/signals  /analysis  /all  /status  /perf  /help  /stop"),
    ...
    if txt in ("/start", "/subscribe"):
        ...
        _send1(cid,
            ...
            "/analysis — التحليلات التفصيلية\n"
            "/all      — كل الرسائل\n"
            "/status   — حالة اشتراكك\n"
            "/perf     — أداء كل استراتيجية\n"
            "/stop     — إلغاء الاشتراك\n\n"
            "🔔 <b>نظام الإشارات:</b>\n"
            f"الإشارة تصدر عند توافق {UNI_AGR}+ من 3 استراتيجيات\n"
            "• 3/3 ← إشارة قوية جداً\n"
            "• 2/3 ← إشارة متوسطة\n\n"
            "⚡ <b>المشغّل الذكي:</b>\n"
            f"فحص كل {RUN_M}د — تشغيل فوري عند:\n"
            f"  تغيّر سعر ≥{TRIG_PCH}% | حجم ×{TRIG_VOL} | شمعة جديدة\n"
...
_AI_PROVIDERS = {
    ...
    "ppq": {
        "url":     "https://api.ppq.ai/v1/messages",
...
def ai_analyze(v7: dict, smc: dict, ob: dict, agg: dict, price: float) -> dict:
    ...
    try:
        ...
        if "content" not in data or not data["content"]:
            log.warning(f"[AI] استجابة فارغة من {provider_name}")
            return {**_empty, "provider": provider_name}
...
def msg_signal(v7: dict, smc: dict, ob: dict, agg: dict, trigger_reasons: list) -> str:
    ...
    lines=[
        f"{'📈' if d==1 else '📉'} <b>إشارة ذهب موحّدة v2.0 — XAU/USD</b>",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
...
def msg_daily(perf: dict, price: float) -> str:
    lines=[
        "📈 <b>التقرير اليومي — بوت الذهب الموحّد v2.0</b>",
...
def main():
    log.info("="*65)
    log.info("🚀 بوت الذهب الموحّد v2.0 — يبدأ التشغيل")
    ...
    if tg_ok:
        tg_send(
            "🚀 <b>بوت الذهب الموحّد v2.0 بدأ!</b>\n"
            f"📌 {SYMBOL}  |  {trading_session()}  |  {utcnow_str()}\n\n"
            "📡 <b>3 استراتيجيات مطوّرة:</b>\n"
            "  ① V7  — 10 مؤشرات + Divergence + Ichimoku + OBV\n"
            "  ② SMC — BOS/CHoCH/MSS + Liquidity + Premium/Discount\n"
            "  ③ OBinOB — كتل الأوامر HTF داخل LTF\n"
            "  ④ انعكاس جلسة آسيا — CHoCH/BOS + 50% Retracement\n"
            "  ④ Claude AI — محلّل ذكي اختياري\n\n"
            "🎯 <b>ثلاثة أهداف:</b> TP1 / TP2 / TP3\n"
            f"⚡ <b>مشغّل ذكي:</b> فحص كل {RUN_M}د — تشغيل فوري عند أي حدث مهم\n\n"
            f"⚙️ إشارة عند توافق {UNI_AGR}+ من 3 | أرسل /help للمزيد"
        )
