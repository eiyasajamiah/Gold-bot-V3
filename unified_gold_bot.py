...
def _find_range_levels(df: pd.DataFrame, at: pd.Series) -> Optional[dict]:
    """
    يبحث عن مستويات دعم ومقاومة متكررة تشكّل نطاقاً واضحاً.
    يعيد: resistance, support, touches_r, touches_s, width_atr, width_pct
    """
    n   = min(RANGE_LOOKBACK, len(df))
    sub = df.tail(n).reset_index(drop=True)
    hi  = sub["High"].values
    lo  = sub["Low"].values
    avg_at = float(at.tail(n).mean())
    if avg_at == 0: return None

    tol_hi = float(sub["High"].max()) * RANGE_TOLERANCE_PCT
    tol_lo = float(sub["Low"].max())  * RANGE_TOLERANCE_PCT

    # تجميع القمم والقيعان القريبة
    def cluster(vals, tol):
        groups = []
        for v in sorted(vals):
            found = False
            for g in groups:
                if abs(v - g["center"]) <= tol:
                    g["vals"].append(v); g["center"] = sum(g["vals"])/len(g["vals"])
                    found = True; break
            if not found: groups.append({"center":v,"vals":[v]})
        return groups

    r_groups = cluster(hi, tol_hi)
    s_groups = cluster(lo, tol_lo)

    # أفضل مقاومة: أكثر لمسات + أقرب للسعر الحالي من الأعلى
    price = float(sub["Close"].iloc[-1])
    r_cands = [g for g in r_groups if len(g["vals"]) >= RANGE_MIN_TOUCHES and g["center"] > price*0.998]
    s_cands = [g for g in s_groups if len(g["vals"]) >= RANGE_MIN_TOUCHES and g["center"] < price*1.002]

    if not r_cands or not s_cands: return None

    # أقرب مقاومة فوق السعر
    resistance = min(r_cands, key=lambda g: abs(g["center"]-price))
    # أقرب دعم تحت السعر
    support    = min(s_cands, key=lambda g: abs(g["center"]-price))

    R = resistance["center"]; S = support["center"]
    if R <= S: return None

    width     = R - S
    width_atr = width / avg_at
    width_pct = width / S * 100

    if width_atr < RANGE_MIN_WIDTH_ATR or width_atr > RANGE_MAX_WIDTH_ATR:
        return None

    # نوع النطاق
    if width_atr < 2.0:   rtype = "ضيق 🔵"
    elif width_atr < 4.0: rtype = "متوسط 🟡"
    else:                  rtype = "واسع 🟠"
    ...
    return {
        ...
        "touches_r":   len(resistance["vals"]),
        "touches_s":   len(support["vals"]),
        "type":        rtype,
        "avg_atr":     round(avg_at, 2),
    }


def _confirm_range(df: pd.DataFrame, rng: dict) -> dict:
    """
    يتحقق من شروط تأكيد النطاق:
    - ADX < 25 (سوق جانبي)
    - RSI في المنتصف أو قرب حدود النطاق
    - حجم منخفض نسبياً (تداول جانبي)
    - السعر داخل النطاق
    """
    ...
    inside_range = S <= price <= R
    near_support = price <= S * (1 + RANGE_ENTRY_BUFFER * 3)
    near_resist  = price >= R * (1 - RANGE_ENTRY_BUFFER * 3)
    range_valid  = adx_now < RANGE_ADX_MAX and inside_range

    # خطر الاختراق: السعر اقترب جداً من أحد الحدين
    break_risk_up   = price >= R * (1 - RANGE_BREAK_BUFFER)
    break_risk_down = price <= S * (1 + RANGE_BREAK_BUFFER)
    break_risk      = break_risk_up or break_risk_down
    ...
    return {
        ...
        "break_risk_up":   break_risk_up,
        "break_risk_down": break_risk_down,
        "price":        round(price, 2),
        "pos_pct":      round((price - S) / (R - S) * 100, 1),  # موقع السعر في النطاق %
    }


def _range_entry(rng: dict, conf: dict, at_val: float) -> Optional[dict]:
    """
    يحسب نقطة الدخول والأهداف والحماية.
    الدخول: عند الدعم (شراء) أو المقاومة (بيع)
    TP1: منتصف النطاق | TP2: الطرف الآخر | TP3: امتداد 1.27 خارج النطاق
    """
    R   = rng["resistance"]
    S   = rng["support"]
    mid = rng["mid"]
    ...
    if conf["near_support"] and not conf["break_risk_down"]:
        side  = 1      # شراء
        entry = round(S * (1 + RANGE_ENTRY_BUFFER), 2)
        sl    = round(S - at_val * RANGE_SL_ATR, 2)
        tp1   = round(mid, 2)
        tp2   = round(R * (1 - RANGE_ENTRY_BUFFER), 2)
        tp3   = round(R + W * 0.27, 2)

    elif conf["near_resist"] and not conf["break_risk_up"]:
        side  = -1     # بيع
        entry = round(R * (1 - RANGE_ENTRY_BUFFER), 2)
        sl    = round(R + at_val * RANGE_SL_ATR, 2)
        tp1   = round(mid, 2)
        tp2   = round(S * (1 + RANGE_ENTRY_BUFFER), 2)
        tp3   = round(S - W * 0.27, 2)
    else:
        return None
...
def analyze_range() -> dict:
    """
    الوحدة الرئيسية — يجلب البيانات، يكشف النطاق، يؤكده، يحسب الإشارة.
    يعمل على إطار 1h بشكل أساسي مع تأكيد من 4h.
    """
    _empty = {
        "vote":0,"active":False,"range":None,"conf_data":None,
        "entry":None,"sl":None,"tp1":None,"tp2":None,"tp3":None,
        "rr":None,"reasons":[],"score":0,"confidence":0,
        "analysis_txt":"",
    }
    ...
    at1h = atr_fn(df1h)
    at_val = float(at1h.iloc[-1])

    # ── كشف النطاق على 1h ──
    rng = _find_range_levels(df1h, at1h)
    if rng is None:
        log.debug("[Range] لا يوجد نطاق واضح على 1h")
        return {**_empty, "analysis_txt":"لا يوجد نطاق واضح حالياً"}

    # ── تأكيد النطاق على 4h (اختياري) ──
    rng4h = None
    if df4h is not None:
        rng4h = _find_range_levels(df4h, atr_fn(df4h))

    # ── تأكيد الشروط ──
    cd = _confirm_range(df1h, rng)

    score = 0; reasons = []

    if not cd["inside"]:
        return {**_empty,"range":rng,"conf_data":cd,
                "analysis_txt":f"السعر خارج النطاق {rng['support']}–{rng['resistance']}"}

    if cd["break_risk"]:
        direction = "↑ صاعد" if cd["break_risk_up"] else "↓ هابط"
        reasons.append(f"⚠️ خطر اختراق النطاق {direction} — تجنّب الدخول")
        return {**_empty,"range":rng,"conf_data":cd,"reasons":reasons,
                "analysis_txt":f"⚠️ خطر اختراق {direction}"}

    # ── تقييم جودة النطاق ──
    if cd["adx"] < 20:
        score += 25; reasons.append(f"✅ ADX={cd['adx']:.1f} — سوق جانبي قوي")
    elif cd["adx"] < RANGE_ADX_MAX:
        score += 15; reasons.append(f"✅ ADX={cd['adx']:.1f} — سوق جانبي")
    else:
        reasons.append(f"⚠️ ADX={cd['adx']:.1f} — ترند محتمل")

    if rng["touches_r"] >= 3: score += 15; reasons.append(f"✅ مقاومة قوية ({rng['touches_r']} لمسات)")
    elif rng["touches_r"] >= 2: score += 8
    if rng["touches_s"] >= 3: score += 15; reasons.append(f"✅ دعم قوي ({rng['touches_s']} لمسات)")
    elif rng["touches_s"] >= 2: score += 8

    # تأكيد 4h
    if rng4h:
        if abs(rng4h["resistance"] - rng["resistance"]) / rng["resistance"] < 0.005:
            score += 10; reasons.append(f"✅ مقاومة 4h تتوافق مع 1h ({rng4h['resistance']:.2f})")
        if abs(rng4h["support"] - rng["support"]) / rng["support"] < 0.005:
            score += 10; reasons.append(f"✅ دعم 4h يتوافق مع 1h ({rng4h['support']:.2f})")

    # موقع السعر في النطاق
    pos = cd["pos_pct"]
    reasons.append(f"📍 السعر عند {pos:.1f}% من النطاق ({cd['price']:.2f})")
    reasons.append(f"📐 النطاق: {rng['support']:.2f} ← {rng['mid']:.2f} → {rng['resistance']:.2f}")
    reasons.append(f"📏 عرض النطاق: {rng['width']:.2f}$ ({rng['width_atr']:.1f}×ATR) — {rng['type']}")

    # RSI
    if cd["near_support"] and cd["rsi"] < RANGE_RSI_MID_HIGH:
        score += 10; reasons.append(f"✅ RSI={cd['rsi']:.1f} — قرب الدعم")
    elif cd["near_resist"] and cd["rsi"] > RANGE_RSI_MID_LOW:
        score += 10; reasons.append(f"✅ RSI={cd['rsi']:.1f} — قرب المقاومة")

    # حجم التداول
    if cd["vol_ratio"] < 0.8:
        score += 5; reasons.append(f"✅ حجم منخفض ×{cd['vol_ratio']:.2f} — يؤكد التداول الجانبي")
    elif cd["vol_ratio"] > 1.5:
        score -= 5; reasons.append(f"⚠️ حجم مرتفع ×{cd['vol_ratio']:.2f} — انتبه لاختراق محتمل")

    # ── حساب الإشارة ──
    lvls = _range_entry(rng, cd, at_val)
    if lvls is None or score < 30:
        ana = (f"النطاق: {rng['support']:.2f}–{rng['resistance']:.2f} | "
               f"موقع السعر: {pos:.1f}% | ADX={cd['adx']:.1f}")
        return {**_empty,"range":rng,"conf_data":cd,"reasons":reasons,
                "score":score,"analysis_txt":ana}

    vote = lvls["side"]
    conf = min(95, score + int(min(lvls["rr2"]*5, 15)))

    side_ar = "شراء 🟢 (ارتداد من الدعم)" if vote==1 else "بيع 🔴 (ارتداد من المقاومة)"
    reasons.insert(0, f"🎯 {side_ar}")

    ana_txt = (
        f"النطاق: {rng['support']:.2f}–{rng['resistance']:.2f} ({rng['type']}) | "
        f"دخول: {lvls['entry']:.2f} | SL: {lvls['sl']:.2f} | "
        f"TP1: {lvls['tp1']:.2f} | TP2: {lvls['tp2']:.2f} | R:R={lvls['rr2']:.1f}"
    )

    log.info(f"[Range] vote={vote} score={score} conf={conf}% "
             f"R={rng['resistance']} S={rng['support']} pos={pos:.1f}%")
    ...
    return {
        ...
        "score":        score,
        "confidence":   conf,
        "analysis_txt": ana_txt,
    }


# ══════════════════════════════════════════════════════════════════
# MODULE 5 — Range Strategy (استراتيجية النطاق)
# ══════════════════════════════════════════════════════════════════
# المنطق:
#   1. كشف نطاق الدعم/المقاومة خلال آخر N شمعة
#   2. تحديد حدود النطاق العلوي (مقاومة) والسفلي (دعم)
#   3. إشارة شراء: السعر عند الدعم + تأكيد انعكاس
#   4. إشارة بيع:  السعر عند المقاومة + تأكيد انعكاس
#   5. كشف كسر النطاق (Breakout) — إشارة استمرارية

RANGE_LOOKBACK   = 50    # عدد الشموع لتحديد النطاق
RANGE_TOUCH_MIN  = 2     # أدنى عدد لمسات لتأكيد المستوى
RANGE_BUFFER_PCT = 0.003 # 0.3% هامش حول الحدود
RANGE_MIN_WIDTH  = 0.004 # عرض النطاق الأدنى (0.4% من السعر)
...
def main():
    ...
    while True:
        ...
        log.info("⑤ Range...")
        _re={"vote":0,"active":False,"range":None,"conf_data":None,"entry":None,"sl":None,"tp1":None,"tp2":None,"tp3":None,"rr":None,"reasons":[],"score":0,"confidence":0,"analysis_txt":""}
        ...
        if agg["d"]!=0 and agg["entry"] is not None:
            ...
            tg_send(msg_analysis(v7,smc,ob,smc.get("price",cur_price),asia,rng),flt=S_ANA)
