...
def merge_with_ai(agg: dict, ai: dict) -> dict:
    ...
    if d_fin and entry is not None:
        if d_fin == 1:
            if tp1 is not None: tp1 = min(tp1, entry + MAX_TP * 0.35)
            if tp2 is not None: tp2 = min(tp2, entry + MAX_TP * 0.65)
            if tp3 is not None: tp3 = min(tp3, entry + MAX_TP)
        else:
            if tp1 is not None: tp1 = max(tp1, entry - MAX_TP * 0.35)
            if tp2 is not None: tp2 = max(tp2, entry - MAX_TP * 0.65)
            if tp3 is not None: tp3 = max(tp3, entry - MAX_TP)
...
def aggregate(v7: dict, smc: dict, ob: dict, asia: dict = None, rng: dict = None, fib: dict = None, rev: dict = None, sc: dict = None) -> dict:
    ...
    if d and entry is not None:
        if d == 1:  # شراء
            ...
            if tp2 is not None: tp2 = min(tp2, entry + MAX_TP * 0.65)
            if tp3 is not None: tp3 = min(tp3, entry + MAX_TP)
            if tp  is not None: tp  = min(tp,  entry + MAX_TP * 0.65)
        else:        # بيع
            if tp1 is not None: tp1 = max(tp1, entry - MAX_TP * 0.35)
            if tp2 is not None: tp2 = max(tp2, entry - MAX_TP * 0.65)
            if tp3 is not None: tp3 = max(tp3, entry - MAX_TP)
            if tp  is not None: tp  = max(tp,  entry - MAX_TP * 0.65)
        # إعادة حساب R:R بعد تعديل الأهداف
        if sl is not None and tp2 is not None:
            rr = round(abs(tp2 - entry) / (abs(sl - entry) + 1e-9), 2)
